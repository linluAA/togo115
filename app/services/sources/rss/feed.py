...
from app.services.link import first_text, item_context, item_links, extract_download_links
...
class RssTorznabFeedMixin:
    def _parse_feed(self, source: dict[str, Any], xml_text: str) -> list[SearchResult]:
        ...
        for item in items:
            title = first_text(item, ("title",)) or name
            ...
            links = item_links(item, allow_direct_http=source_type == "torznab") or extract_download_links(text)
            for link in links:
                results.append(
                    SearchResult(
                        title=title[:120],
                        ...
                        message_id=first_text(item, ("guid", "id")),
                        context=text,
                    )
                )
        return results
