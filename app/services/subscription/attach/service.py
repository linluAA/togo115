...
async def attach_results_to_matching_subscriptions(
    ...
    ordered_results = sorted(
        results,
        key=lambda result: (-result_priority(result), str(getattr(result, "source", "")), str(getattr(result, "message_id", ""))),
    )
    ordered_results = await filter_available_115_results(ordered_results)
    for subscription in subscriptions:
        if not results_may_match_subscription(subscription, ordered_results, message_text):
            continue
...
def _attach_results_for_subscription(subscription: dict, ordered_results: list[SearchResult], resource_ids: list[int]) -> int:
    ...
    with db() as conn:
        existing_rows = existing_resource_rows(conn, int(subscription["id"]))
        existing_115 = subscription_115_resources(conn, int(subscription["id"]))
        for result in ordered_results:
            is_fallback = result_is_fallback_source(result)
            if is_fallback and fallback_attached:
                continue
            if is_fallback and fallback_matched_priority is not None and result_priority(result) < fallback_matched_priority:
                continue
            ...
            if _fallback_is_blocked(conn, subscription, result, existing_115):
                if is_fallback and fallback_matched_priority is None:
                    fallback_matched_priority = result_priority(result)
                continue
