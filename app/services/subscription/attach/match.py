...
from app.services.subscription.resource.ops import fallback_blocked_by_primary_resource
...
def _result_matches_subscription(subscription: dict, result: SearchResult) -> bool:
    try:
        ...
        add_log(
            ...
            "subscription",
            "\u5b9e\u65f6\u8d44\u6e90\u5339\u914d\u5224\u65ad\u5f02\u5e38\uff0c\u5df2\u8df3\u8fc7\u5355\u6761\u7ed3\u679c",
            {"id": subscription["id"], **result_debug_payload(result), "error": str(exc)},
        )
        return False
...
def _fallback_is_blocked(conn, subscription: dict, result: SearchResult, existing_115: list[dict[str, Any]]) -> bool:
    try:
        blocked = fallback_blocked_by_primary_resource(conn, subscription, result, existing_115)
    except Exception as exc:
        add_log(
            "warning",
            "subscription",
