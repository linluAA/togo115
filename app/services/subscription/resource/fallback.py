...
def fallback_blocked_by_primary_resource(
    conn: sqlite3.Connection,
    subscription: dict,
    result: SearchResult,
    existing_115: list[dict[str, Any]] | None = None,
) -> bool:
    if not result_is_fallback_source(result):
        return False
...
def results_may_match_subscription(subscription: dict, results: list[SearchResult], *extra_texts: str) -> bool:
...
def fallback_result_candidates(results: list[SearchResult], subscription: dict | None = None) -> list[SearchResult]:
    return sorted(results, key=lambda result: fallback_candidate_sort_key(subscription, result))


def best_fallback_result(results: list[SearchResult], subscription: dict | None = None) -> SearchResult | None:
    candidates = fallback_result_candidates(results, subscription)
