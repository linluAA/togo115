...
def decide_resource_candidate(subscription: dict, result: SearchResult, *extra_texts: str) -> ResourceDecision:
    ...
    episodes = frozenset(_episode_keys_from_text_for_subscription(subscription, _result_text(result, *extra_texts)))
    missing = frozenset(_missing_episode_keys(subscription))
...
def _candidate_reject_reason(
    ...
    *extra_texts: str,
    ...
    if not result_matches_subscription(subscription, result, *extra_texts):
        return _result_skip_reason(subscription, result, *extra_texts) or "subscription_mismatch"
    if not result_matches_missing_episodes(subscription, result, *extra_texts):
        ...
        return _result_skip_reason(subscription, result, *extra_texts) or "missing_episode_mismatch"
    return ""
...
def _candidate_score(
    ...
    score += min(max(_result_priority(result), -50), 50)
...
def _quality_preference_score(subscription: dict | None, result: SearchResult) -> int:
    ...
    if preferred_quality and _text_contains_any(_result_text(result), preferred_quality):
        return 1
