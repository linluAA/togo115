...
def _expected_bundle(files: list[str], source_dir: Path, root: Path, header: str, *, minify: bool = False) -> str:
    chunks = [header.rstrip(), ""]
    if minify:
        for name in files:
            path = source_dir / name
            raw = path.read_text(encoding="utf-8").strip()
            chunks.append(build_static._minify_js(raw).rstrip())
            chunks.append("")
    else:
        for name in files:
            path = source_dir / name
...
def test_static_bundles_are_generated_from_split_sources():
    ...
    assert (build_static.STATIC / "app.js").read_text(encoding="utf-8") == _expected_bundle(
        build_static.JS_FILES, build_static.JS_SRC, build_static.ROOT,
        "// Generated by app/tools/build_static.py. Edit app/static/src/js/* instead.",
        minify=True,
    ) + "\n"
