...
- 后台监控：定时检查 Telegram/TG Bot 监听状态并同步 Emby 入库状态；创建订阅或手动点击搜索时才执行 Telegram 历史搜索。
...
系统会在创建订阅或手动搜索时用 `iter_messages(search=...)` 搜索历史消息；实时追新依赖 Telegram 新消息监听。如果消息带按钮，会尝试点击包含 `115`、`链接`、`查看`、`打开`、`资源`、`link` 的按钮并从响应里提取 115 分享链接。

## 订阅源配置

设置里的“订阅源”支持三种类型：

- `RSS`：填写 RSS URL，系统定时读取条目并提取 115、magnet、torrent 链接。
- `Torznab`：填写 Torznab API URL，可使用 `{query}` 占位符，或让系统自动补 `t=search&q=剧名`。
- `磁力网页`：填写搜索 URL 模板，例如 `https://yhdm33.com/s/{query}.html`。系统会先打开搜索页，再进入同站内疑似详情页，提取 `magnet` / `.torrent` 链接。

每个订阅源可以填写“测试关键词”，点击测试时会用该关键词替换 `{query}`；未填写时会回退使用源名称。
从 TMDB 添加订阅时会保存首播/上映年份；磁力网页搜索页如果能识别结果年份，会优先只进入同年份详情页，并在资源匹配时拒绝明确标注为其它年份的结果。

磁力网页源依赖目标网站页面结构，站点改版时可能需要调整适配规则。磁力或 torrent 链接无法直接转存到 115，建议把全局推送方式设置为“发送到 TG Bot”。
